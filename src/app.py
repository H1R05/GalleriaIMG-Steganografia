...
class GalleriaImmagini(ttk.Window):
    ...
    def _create_details_panel(self, parent):
        ...
        details_frame_outer = ttk.LabelFrame(parent, text="Dettagli / Steganografia", padding=(10, 5), bootstyle=INFO)
...
# Questo codice viene eseguito solo se lo script è lanciato direttamente (non importato)
if __name__ == "__main__":
    ...
    try:
        ...
        print("Avvio mainloop Tkinter...")
        # Avvia il ciclo principale di eventi Tkinter (rende la finestra interattiva)
        app.mainloop()
        print("Mainloop Tkinter terminato.")
    except Exception as e: # Cattura errori critici non gestiti
        error_msg = f"Errore critico non gestito:\n\n{traceback.format_exc()}"
        print(error_msg)
        # Prova a mostrare l'errore in un popup (potrebbe fallire se Tkinter è compromesso)
        try:
            root = tk.Tk(); root.withdraw() # Crea finestra nascosta per popup
            messagebox.showerror("Errore Critico", error_msg); root.destroy()
        except Exception: pass # Ignora se anche il popup fallisce
    finally:
